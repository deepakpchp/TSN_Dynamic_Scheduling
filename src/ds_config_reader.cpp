...
int configuration::read_flow_config(){
		...
		if (0 < this->num_of_flows)
		{
			...
			this->route_length = new int[this->num_of_flows];
			...
			while(getline(cFile, line)){
				...
				if("True" == tokens[5]){
					...
					this->route_length[flow_index] = tokens.size() - 6;

					this->route[flow_index] = new int[this->route_length[flow_index]];
					this->route_queue_assignment[flow_index] = new int[this->route_length[flow_index]];
					this->queue_state[flow_index] = new link::queue_reservation_state[this->route_length[flow_index]];
					...
					for (unsigned int token_index = 0; token_index <  tokens.size() - 6; token_index++){
						...
						this->route[flow_index][token_index] = stoi(reservation_details[0]);
						this->route_queue_assignment[flow_index][token_index] = stoi(reservation_details[1]);
						if("OPEN" == reservation_details[2]){
...
int configuration::reset_reserv_config(int flow_index){
	...
	this->reservation_availability[flow_index] = false;
	this->route_length[flow_index] = 0;
...
int configuration::get_route_length(int index){
	return this->route_length[index];
}
